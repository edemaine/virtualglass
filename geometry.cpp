...
void Geometry::save_raw_file(std::string const &filename) const {
	...
	{ //write to temp file:
		...
		for (vector< Group >::const_iterator g = groups.begin(); g != groups.end(); ++g) {
			...
			assert(g->begin + g->size <= triangles.size());
			vector< Triangle > group_tris(triangles.begin() + g->begin, triangles.begin() + g->begin + g->size);
			assert(group_tris.size() == g->size);
			...
			{ //write group color:
				...
				Vector4f skin_color = make_vector(g->cane->color.xyz, 1.0f);
				//Filter-per-unit-length:
				Vector3f filter_after_unit = g->cane->color.xyz * g->cane->color.w;
